...
/*------------------------------------------------------------------------
  ...
  SDCCralloc.c - source file for register allocation. (PBlaze) specific
  
  Written By -  If I only knew... stole it from XA51 port and bent it to my needs
...
regs regsPBlaze[] = {
    ...
    {S0_ID, 1, REG_GPR, "s0", 0x0000, 1, NULL},
    {S1_ID, 1, REG_GPR, "s1", 0x0001, 1, NULL},
    {S2_ID, 1, REG_GPR, "s2", 0x0002, 1, NULL},
    {S3_ID, 1, REG_GPR, "s3", 0x0003, 1, NULL},
    {S4_ID, 1, REG_GPR, "s4", 0x0004, 1, NULL},
    {S5_ID, 1, REG_GPR, "s5", 0x0005, 1, NULL},
    {S6_ID, 1, REG_GPR, "s6", 0x0006, 1, NULL},
    {S7_ID, 1, REG_GPR, "s7", 0x0007, 1, NULL},
    {S8_ID, 1, REG_GPR, "s8", 0x0008, 1, NULL},
    {S9_ID, 1, REG_GPR, "s9", 0x0009, 1, NULL},
    {SA_ID, 1, REG_GPR, "sA", 0x000A, 1, NULL},
    {SB_ID, 1, REG_GPR, "sB", 0x000B, 1, NULL},
    {SC_ID, 1, REG_GPR, "sC", 0x000C, 1, NULL},
    {SD_ID, 1, REG_GPR, "sD", 0x000D, 1, NULL},
    {SE_ID, 1, REG_GPR, "sE", 0x000E, 1, NULL},
    {SF_ID, 1, REG_GPR, "sF", 0x000F, 1, NULL},
};

int PBlaze_nRegs = sizeof(regsPBlaze) / sizeof(regs);

// the currently in use registers
unsigned long PBlazeRegsInUse = 0;

// this should be set with a command line switch
bool PBlazeHasGprRegs = 0;
...
checkRegMask(const char *f) {   // for debugging purposes only
    ...
    // rebuild the regmask
    for (i = 0; i < PBlaze_nRegs; i++) {
        if (!regsPBlaze[i].isFree) {
            regMask |= regsPBlaze[i].regMask;
        }
    }

    // check it
    if (regMask != PBlazeRegsInUse) {
        fprintf(stderr,
                "error(%s): regMask inconsistent 0x%08lx != 0x%08lx\n", f,
                regMask, PBlazeRegsInUse);
        regMask = regMask ^ PBlazeRegsInUse;
        fprintf(stderr, "%s used by %s\n",
                PBlaze_regWithMask(regMask)->name,
                PBlaze_regWithMask(regMask)->sym->name);
...
freeReg(regs * reg, bool silent) {
    ...
    if (!silent) {
        D0(fprintf(stderr, "freeReg: (%08lx) %s (%s) ", PBlazeRegsInUse,
                   reg->name, reg->sym->name));
    }

    if (reg->isFree || ((PBlazeRegsInUse & reg->regMask) != reg->regMask)) {
        werror(E_INTERNAL_ERROR, __FILE__, __LINE__,
               "freeReg - freeing unused register(s)");
        exit(1);
    }
    PBlazeRegsInUse &= ~reg->regMask;
    reg->isFree = 1;
    reg->sym = NULL;
    if (!silent)
...
/*-----------------------------------------------------------------*/
static bool
allocReg(unsigned int size, int type, symbol * sym, int offset, bool silent) {
    int i;
    ...
    if (!silent) {
        D0(fprintf(stderr, "allocReg (%08lx) for %s size:%d, type:%s ",
                   PBlazeRegsInUse, sym->name, size, regTypeToStr(type)));
    }
    ...
    case 1:
        ...
        for (i = 0; i < PBlaze_nRegs; i++) {
            if (regsPBlaze[i].size == 2 && regsPBlaze[i].type == type) {
                unsigned long mask = regsPBlaze[i].regMask & ~PBlazeRegsInUse;
                if (mask && mask != regsPBlaze[i].regMask) {
                    regs *reg = PBlaze_regWithMask(mask);
                    // found a gap
                    sym->regs[offset] = reg;
                    PBlazeRegsInUse |= mask;
    ...
    case 2:
        for (i = 0; i < PBlaze_nRegs; i++) {
            if (regsPBlaze[i].size == size &&
                regsPBlaze[i].type == type &&
                (regsPBlaze[i].regMask & PBlazeRegsInUse) == 0) {
                PBlazeRegsInUse |= regsPBlaze[i].regMask;
                regsPBlaze[i].isFree = 0;       // redundant
                regsPBlaze[i].sym = sym;
                if (!silent) {
                    D0(fprintf(stderr, "%s\n", regsPBlaze[i].name));
                }
                sym->regs[offset] = &regsPBlaze[i];
                checkRegMask(__FUNCTION__);
                return TRUE;
            }
        }
    ...
    case 4:                    // this is a dword
        ...
        if ((PBlazeHasGprRegs && allocReg(2, REG_GPR, sym, offset, silent)) ||
            allocReg(2, REG_PTR, sym, offset, silent)) {
            if ((PBlazeHasGprRegs
                 && allocReg(2, REG_GPR, sym, offset + 1, silent))
...
freeAllRegs(void) {
    ...
    for (i = 0; i < PBlaze_nRegs; i++) {
        if (!regsPBlaze[i].isFree) {
            strcat(regsFreed, regsPBlaze[i].name);
            strcat(regsFreed, " ");
            regsPBlaze[i].isFree = 1;
            regsPBlaze[i].sym = NULL;
            nfr++;
        }
    }
    PBlazeRegsInUse = 0;
...
getRegPtr(iCode * ic, eBBlock * ebp, symbol * sym, short offset) {
    ...
    while (1) {
        ...
        /* try for gpr type */
        if (PBlazeHasGprRegs && allocReg(getSize(sym->type),
...
willCauseSpill(symbol * sym) {
    ...
    if (allocReg(getSize(sym->type), sym->regType, sym, 0, TRUE) ||
        allocReg(getSize(sym->type),
                 sym->regType == REG_PTR ? REG_GPR : REG_PTR, sym, 0, TRUE)) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (ic = ebbs[i]->sch; ic; ic = ic->next) {
            ...
            if (SKIP_IC2(ic) ||
                ...
                ic->op == IFX ||
                ic->op == IPUSH ||
                ic->op == IPOP || (IC_RESULT(ic) && POINTER_SET(ic)))
                continue;
            ...
            if (IC_RESULT(ic)) {
                ...
                _G.totRegAssigned =
                    bitVectSetBit(_G.totRegAssigned, sym->key);
...
PBlaze_rUmaskForOp(operand * op) {
    ...
    rumask = newBitVect(PBlaze_nRegs);
...
regsUsedIniCode(iCode * ic) {
    ...
    if (ic->op == IFX) {
        rmask = bitVectUnion(rmask, PBlaze_rUmaskForOp(IC_COND(ic)));
        goto ret;
    }

    /* for the jumptable */
    if (ic->op == JUMPTABLE) {
        rmask = bitVectUnion(rmask, PBlaze_rUmaskForOp(IC_JTCOND(ic)));
    ...
    /* of all other cases */
    if (IC_LEFT(ic))
        rmask = bitVectUnion(rmask, PBlaze_rUmaskForOp(IC_LEFT(ic)));


    if (IC_RIGHT(ic))
        rmask = bitVectUnion(rmask, PBlaze_rUmaskForOp(IC_RIGHT(ic)));

    if (IC_RESULT(ic))
        rmask = bitVectUnion(rmask, PBlaze_rUmaskForOp(IC_RESULT(ic)));
...
createRegMask(eBBlock ** ebbs, int count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (ic = ebbs[i]->sch; ic; ic = ic->next) {
            ...
            ic->rMask = newBitVect(PBlaze_nRegs + 1);
    ...
    for (sym = hTabFirstItem(liveRanges, &k); sym;
        ...
        if (sym->isitmp) {
            ...
            if (bitVectnBitsOn(sym->defs) == 1 &&
                ...
                POINTER_GET(ic) &&
                !sym->noSpilLoc && !IS_BITVAR(sym->etype)) {
                /* and that pointer is remat in data space */
                if (OP_SYMBOL(IC_LEFT(ic))->remat &&
                    !IS_CAST_ICODE(OP_SYMBOL(IC_LEFT(ic))->rematiCode) &&
                    DCL_TYPE(aggrToPtr(operandType(IC_LEFT(ic)), FALSE)) ==
                    POINTER) {
                    /* create a psuedo symbol & force a spil */
                    symbol *psym =
                        newSymbol(rematStr(OP_SYMBOL(IC_LEFT(ic))), 1);
                    psym->type = sym->type;
...
packRegsForAssign(iCode * ic, eBBlock * ebp) {
    ...
    OP_DEFS(IC_RESULT(dic)) =
        bitVectSetBit(OP_DEFS(IC_RESULT(dic)), dic->key);
    return 1;
...
packRegsForOneuse(iCode * ic, operand * op, eBBlock * ebp) {
    ...
    if (ic->op != RETURN &&
        ic->op != SEND && !POINTER_SET(ic) && !POINTER_GET(ic))
        return NULL;
            ...
            if (IS_INTEGRAL(fromType) && IS_INTEGRAL(toType) &&
                ...
                if (dic) {
                    if (IS_ARITHMETIC_OP(dic)) {
                        bitVectUnSetBit(OP_SYMBOL(IC_RESULT(dic))->defs,
                                        dic->key);
                        IC_RESULT(dic) = IC_RESULT(ic);
                        remiCodeFromeBBlock(ebp, ic);
                        bitVectUnSetBit(OP_SYMBOL(IC_RESULT(ic))->defs,
                                        ic->key);
                        hTabDeleteItem(&iCodehTab, ic->key, ic, DELETE_ITEM,
                                       NULL);
                        OP_DEFS(IC_RESULT(dic)) =
                            bitVectSetBit(OP_DEFS(IC_RESULT(dic)), dic->key);
                        ic = ic->prev;
...
pblaze_assignRegisters(ebbIndex * ebbi) {
    ...
    recomputeLiveRanges(ebbs, count, true);
    ...
    if (_G.stackExtend) {
        werror(I_EXTENDED_STACK_SPILS, _G.stackExtend, currFunc->name, "");
        _G.stackExtend = 0;
    }

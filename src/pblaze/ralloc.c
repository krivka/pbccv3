...
static bool allocReg (unsigned int size, int type, symbol *sym, 
  ...
  switch (size) 
	...
	if ((PBlazeHasGprRegs && allocReg (2, REG_GPR, sym, offset+1, silent)) ||
...
regsUsedIniCode (iCode * ic)
{
  ...
  if (ic->op == JUMPTABLE)
    {
      rmask = bitVectUnion (rmask,
			    PBlaze_rUmaskForOp (IC_JTCOND (ic)));
  ...
  if (IC_LEFT (ic))
    rmask = bitVectUnion (rmask,
			  PBlaze_rUmaskForOp (IC_LEFT (ic)));
  
  
  if (IC_RIGHT (ic))
    rmask = bitVectUnion (rmask,
			  PBlaze_rUmaskForOp (IC_RIGHT (ic)));
  
  if (IC_RESULT (ic))
    rmask = bitVectUnion (rmask,
			  PBlaze_rUmaskForOp (IC_RESULT (ic)));
